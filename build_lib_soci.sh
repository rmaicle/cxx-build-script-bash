...
# SOCI library build script for GNU/Linux
#
# References:
# https://github.com/SOCI/soci
# Documentation: http://soci.sourceforge.net/doc/release/4.0/
# Installation: https://dane-bulat.medium.com/working-with-databases-in-c-an-introduction-7d6a6a78ae66
# On MS Windows: https://stackoverflow.com/questions/28453993/include-soci-in-cmake-on-windows
#
# Example:
#   ./build_lib_soci.sh         \
#       --compiler=gcc:10.3.0   \
...
$(printf '                          %s\n' ${CXX_STANDARDS[@]})
  ...
  -v, --version=x.y.z  library version (x.y.z); default is ${LIB_SOCI_VERSION_DEFAULT} or use
                       one of
$(printf '                         %s\n' ${LIB_SOCI_VERSIONS[@]})
EOF
}
...
declare arg_version="${LIB_SOCI_VERSION_DEFAULT}"
...
while true; do
    case "${1}" in
        ...
        -v|--version)       arg_version="${2}"
                            shift 2
                            check_arg_in_array "-b|--build" "${arg_version}" "LIB_SOCI_VERSIONS"
                            arg_version=$(get_id "${arg_version}" "LIB_SOCI_VERSIONS")
                            ;;
        *)                  break ;;
    esac
done
...
Flags:
  ...
  Link:          ${v_link_flags}
Directories:
  Extract:       ${DIR_SOCI_EXTRACT}
  Build:         ${DIR_SOCI_BUILD}
  Staging:       ${DIR_SOCI_STAGING}
...
extract_soci_archive flag_initial_build
if [ ${flag_info_only} -gt 0 ]; then
    echo "${v_info_text}"
    exit
fi
recreate_soci_build_and_staging_dirs ${flag_rebuild}

pushd "${DIR_SOCI_BUILD}"
declare v_generator_type="Unix Makefiles"
declare v_system_name="Linux"
[[ "${OSTYPE}" == "${OSTYPE_LINUX}"* ]] && v_system_name="Linux"
[[ "${OSTYPE}" == "${OSTYPE_MSYS}" ]] && v_system_name="MSYS"
...
if [[ ${flag_initial_build} -gt 0 ]]; then
    ...
    cmake                                                                       \
        ...
        -D CMAKE_INSTALL_PREFIX="${DIR_SOCI_STAGING}"                           \
        -D SOCI_CXX11=ON                                                        \
        -D SOCI_SHARED=ON                                                       \
        -D SOCI_STATIC=OFF                                                      \
        -D SOCI_TESTS=OFF                                                       \
        -D SOCI_VISIBILITY=OFF                                                  \
        -D WITH_BOOST=OFF                                                       \
        -D WITH_DB2=OFF                                                         \
        -D WITH_FIREBIRD=OFF                                                    \
        -D WITH_MYSQL=OFF                                                       \
...
pushd "${DIR_SOCI_STAGING}"
[[ -f "${LIB_SOCI}-build.config" ]] && rm -f ${LIB_SOCI}-build.config
echo "${v_info_text}" > ${LIB_SOCI}-build.config
popd
